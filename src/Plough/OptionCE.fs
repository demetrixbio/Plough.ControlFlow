...
module OptionCE =  
    ...
    type OptionBuilder() =
        ...
        member this.Using(resource: 'a when 'a :> IDisposable, binder) : Option<_>=
            this.TryFinally (
                (fun () -> binder resource),
                (fun () -> if not <| obj.ReferenceEquals(resource, null) then resource.Dispose ())
            )

        member this.While(guard: unit -> bool, generator: unit -> Option<_>) : Option<_> =
            if not <| guard () then this.Zero ()
            else this.Bind(this.Run generator, fun () -> this.While (guard, generator))

        member this.For(sequence: #seq<'a>, binder: 'a -> Option<_>) : Option<_> =
